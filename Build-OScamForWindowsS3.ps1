...
try
{
	...
	Write-Host "AUTOMATED OSCAM COMPILER FOR WINDOWS (S3 BASED)"
	Write-Host
	Write-Host "The whole process is fully automated. You only need this script, internet access"
	Write-Host "and at least 3 GB free disk space in the temp directory."
	Write-Host
...
@'
	...
	set CYGWIN_PACKAGES=libssl-devel=1.1.1w-1,openssl=1.1.1w-1,ImageMagick,autoconf,autoconf-archive,automake,bc,bison,curl,dialog,diffstat,dos2unix,flex,gcc-core,gcc-g++,git,git-svn,gperf,help2man,inkscape,jq,libcurl-devel,libglib2.0-devel,libglib2.0_0,libncurses-devel,libreadline-devel,libssl1.0-devel,libtool,libusb1.0-devel,make,mingw64-i686-gcc-core,mingw64-i686-gcc-g++,nano,patch,python3,ruby,sshpass,unzip,upx,vim,wget,zlib,zlib-devel,zip
	...
	set NODEJS_VERSION=latest-v25.x
	...
	set ANSIBLE_GIT_BRANCH=stable-2.20
...
'@ | Out-File -FilePath (-join($tempPath,'\cygwin-portable-installer-config.cmd')) -Force -Encoding ascii
    ...
    $x = @'
        ...
        wget https://raw.githubusercontent.com/h0ly0ne/libdvbcsa/refs/heads/master/patches/libdvbcsa22.patch
        ...
        echo
		echo "GIT CHECKOUT PCSC TRUNK"
		echo "======================================================================"
		git clone https://github.com/h0ly0ne/PCSC.git pcsc-git
		cd pcsc-git
		...
		echo "======================================================================"
		./bootstrap
		./configure --prefix=/usr --disable-libsystemd --disable-polkit --enable-static
		make
		make install
		cd ..
	
		echo
		echo "GIT CHECKOUT CCID TRUNK"
		echo "======================================================================"
		git clone --recursive https://github.com/h0ly0ne/CCID.git ccid-git
		cd ccid-git
		...
		echo "======================================================================"
		./bootstrap
		./configure --prefix=/usr --sysconfdir=/etc --enable-twinserial --enable-static
		make
		make install
		cd ..
        
        echo
        echo "GIT CHECKOUT S3 TRUNK"
        echo "======================================================================"
        git clone https://github.com/h0ly0ne/s3_releases.git s3-git
        cd s3-git
        ...
        mkdir ~/oscam-exe
        cd ~/oscam-exe
		cp ~/s3-git/support/binaries/*cygwin* -t .
		cp ~/s3-git/oscam-git/Distribution/*cygwin* -t .
        for z in $(ls oscam*)
        do
            mv $z oscam.exe
        done
...
'@
    ...
    if ($($PSVersionTable.PSEdition) -ieq 'Core') {
        Set-Content -Value (New-Object System.Text.UTF8Encoding $false).GetBytes(($x -ireplace "`r`n", "`n") + "`n") -AsByteStream -Path (-join($tempPath,'\cygwin\home\root\make.sh')) -Force -NoNewline
    } else {
